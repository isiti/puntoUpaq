<?php
	...
	if (!empty($_POST))
	{
		...
		$sql = "INSERT INTO `travel_logs` (`applicant`, `applicant_company`, `id_users`, `alias_id`, `id_dac`, `travel_date`, `travel_time`, `minutos_anticipacion`, `travel_type`, `from_address`, `lat_from`, `long_from` , `from_address_2`, `from_address_3`, `to_address`, `to_address_2`, `to_address_3`, `id_users_driver`, `amount`, `duration`, `observations`, `observations_2`, `observations_3`, `observations_general`, `message`, `status_dac`, `status`, `by_app`, `fModificacionUsuario`, `fModificacion`, `fCreacion`, `fCreacionUsuario`, `categories`) VALUES ('', '', '".$_SESSION['id']."', ".$_POST['alias'].", 0, '".$_POST['travel_date']."', '".$_POST['travel_time']."', 30, 0, '".$_POST['from_address']."', '".$lati."', '".$longi."' , '', '', '".$_POST['to_address']."', '', '', 0, 0, 0, '', '', '', '".$_POST['comentario']."', '(APP) ".$_POST['message']." ".$hora_mostrar."', 95, 'agendado', 1, '".$_SESSION['id']."', NOW(), NOW(), '".$_SESSION['id']."', 'activo')";
