<?php
	...
	if (isset($_SESSION))
	{
		...
		{
			$players=get_records_db('users',' id_city='.$_SESSION['id_city']." AND id_games = ".$game_id);
		}
		else 
		{
			$players=get_records_db('users',' id_city='.$_SESSION['id_city'].' AND fullname LIKE "%'.$_POST['jugador'].'%"'." AND id_games = ".$game_id);
		}
		...
		foreach ($players as $p) {
			...
			$data_teams .='<div class="play-player-search">
                    ...
                    <div class="player-write">
                        <i class="fa ';
            if (isset($_SESSION['jugadores-invitados']) && isset($_SESSION['jugadores-invitados'][$p['id']]) && $_SESSION['jugadores-invitados'][$p['id']]==1)
